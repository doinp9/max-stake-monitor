// content.js — bridge between injected page script and extension

(function () {
  'use strict';

  // ── injected.js runs in MAIN world automatically via manifest ──
  ...
  function createOverlay() {
    ...
    style.textContent = `
      ...
      #b365-ms-overlay.minimized {
        width: auto;
        border-radius: 6px;
      }
      #b365-ms-overlay.minimized #b365-ms-body {
        display: none;
      }
      #b365-ms-overlay.minimized #b365-ms-title {
        font-size: 12px;
      }
      #b365-ms-header {
        display: flex;
      ...
      #b365-ms-history-list {
        max-height: 150px;
        overflow-y: auto;
      }
      #b365-ms-history-list::-webkit-scrollbar {
        width: 3px;
      }
      #b365-ms-history-list::-webkit-scrollbar-thumb {
        background: #016443;
        border-radius: 2px;
      }
      ...
      .b365-ms-history-item:hover {
        background: #222;
      }
      .b365-ms-history-item .ms-val {
        color: #FFDF00;
        font-weight: 700;
        font-size: 13px;
      }
      .b365-ms-history-item .ms-time {
        color: #555;
        font-size: 10px;
        font-weight: 500;
      }
      .b365-ms-history-item .ms-info {
        color: #999;
        font-size: 10px;
        font-weight: 500;
        max-width: 140px;
    ...
    overlay.querySelector('#b365-ms-close').addEventListener('click', () => {
  ...
  function updateOverlay(data) {
    ...
    historyList.innerHTML = sessionHistory.slice(0, 20).map(item => {
      ...
      return `
        <div class="b365-ms-history-item">
  ...
  window.addEventListener('message', (event) => {
    ...
    console.log('[B365-MS content.js] Received max stake:', data.maxStake, data);

    updateOverlay(data);

    chrome.runtime.sendMessage({
      action: 'NEW_MAX_STAKE',
      data: data,
    });
  });
  ...
  chrome.runtime.onMessage.addListener((msg) => {
    ...
    if (msg.action === 'SET_LANG') {
      currentLang = msg.lang;
      updateOverlayLabels();
    }
  });

  console.log('[B365-MS content.js] Content script loaded ✓');
})();
